...
class ProductCompositeServiceImplTest {
    ...
    private static final String PRODUCT_COMPOSITE = "/product-composite/";
    ...
    void shouldGetProductById() {
        client.get()
                .uri(PRODUCT_COMPOSITE + PRODUCT_ID_OK)
    ...
    void shouldGetProductNotFound() {
        client.get()
                .uri(PRODUCT_COMPOSITE + PRODUCT_ID_NOT_FOUND)
                .accept(APPLICATION_JSON)
                .exchange()
                .expectStatus().isNotFound()
                .expectHeader().contentType(APPLICATION_JSON)
                .expectBody()
                .jsonPath("$.path").isEqualTo(PRODUCT_COMPOSITE + PRODUCT_ID_NOT_FOUND)
                .jsonPath("$.message").isEqualTo("NOT FOUND: " + PRODUCT_ID_NOT_FOUND);
    }

    @Test
    void shouldGetProductInvalidInput() {

        client.get()
                .uri(PRODUCT_COMPOSITE + PRODUCT_ID_INVALID)
                .accept(APPLICATION_JSON)
                .exchange()
                .expectStatus().isEqualTo(UNPROCESSABLE_ENTITY)
                .expectHeader().contentType(APPLICATION_JSON)
                .expectBody()
                .jsonPath("$.path").isEqualTo(PRODUCT_COMPOSITE + PRODUCT_ID_INVALID)
